...
data "aws_kms_secrets" "this" {
  for_each = var.enabled && !var.paused ? var.kms_secrets : {}
  secret {
    name    = each.key
    payload = each.value
  }
...
resource "aws_ssm_parameter" "this" {
  for_each    = var.enabled && !var.paused ? data.aws_kms_secrets.this : {}
...
resource "aws_ssm_parameter" "dns" {
  count       = var.enable_route53 && var.enabled && !var.paused ? 1 : 0
