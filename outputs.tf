...
output "target_group_arn" {
  ...
  value       = var.enabled && !var.paused ? aws_lb_target_group.this[0].arn : ""
}

output "aws_ssm_parameter_names" {
  description = "List of SSM parameter names"
  value       = var.enabled && !var.paused ? [for param in aws_ssm_parameter.this : param.name] : []
}
...
output "route53_health_check_resource_path" {
  ...
  sensitive   = true
  value       = var.enable_route53_health_check && var.enabled && !var.paused ? local.route53_health_check_resource_path : ""
}
...
output "ecs_service_name" {
  ...
  value = var.enabled && !var.paused ? try(aws_ecs_service.this[0].name, null) : null
}
